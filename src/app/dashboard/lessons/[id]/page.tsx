...
export default function LessonDetailPage() {
  ...
  const handleTimeUpdate = () => {
    ...
    const pct =
      (videoRef.current.currentTime / (parseFloat(lesson.duration) * 60)) * 100;
    setWatchedPct(Math.min(100, pct));
    if (pct > 90 && !isCompleted) markComplete("COMPLETED");
  };
      ...
      <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() =>
              markComplete(isCompleted ? "IN_PROGRESS" : "COMPLETED")
            }
            className={`w-full py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 ${
              isCompleted
                ? "bg-emerald-400/10 text-emerald-400 border border-emerald-400/25 hover:bg-emerald-400/15"
                : "bg-gradient-to-r from-ice to-powder text-navy shadow-lg shadow-ice/15 hover:shadow-ice/25"
            }`}
          >
        ...
        <div className="space-y-5">
          ...
          <motion.div
            ...
            className="rounded-2xl bg-gradient-to-br from-[#1e293b] to-[#0f1d30] border border-ice/[0.12] p-5"
          >
            <div className="flex items-center gap-2 mb-3">
